...
public class BluuPurpleTemplate
{
	...
	public int[] QuestIds = {1304, 1306, 1307};
	...
	public void ScriptMain(ScriptInterface bot)
	{
		...
		while (!bot.ShouldExit())
		{
			...
			{			
				MultiQuestFarm("terrarium", "r3", "Right", QuestIds, "*");			
				while (!bot.Inventory.Contains("Super-Fan Swag Token D", 100))
				SafePurchase("Super-Fan Swag Token C", 10, "collection", 325);				
				while (!bot.Inventory.Contains("Super-Fan Swag Token C", 100))
				SafePurchase("Super-Fan Swag Token B", 10, "collection", 325);				
				while (!bot.Inventory.Contains("Super-Fan Swag Token B", 20))
				SafePurchase("Super-Fan Swag Token A", 1, "collection", 325);				
			}

			//SafeQuestComplete(1304); //Bleak Despair
			//SafeQuestComplete(1305); //an apple slice? yum!
			//SafeQuestComplete(1306); //dont drink!
			//SafeQuestComplete(1307); //flying and falling
			//MultiQuestFarm(string MapName, string CellName, string PadName, int[] QuestList, string MonsterName = "*")
