	...
	public void ScriptMain(ScriptInterface bot)
	{
		...
		while (!bot.ShouldExit())
		{
			while (!bot.Player.Loaded) { }
                ...
                bot.Log($"[{DateTime.Now:HH:mm:ss}] do you have the orbs?");
            while (!bot.Inventory.Contains("Spirit Orb", 65000))
            {
                    bot.Log($"[{DateTime.Now:HH:mm:ss}] is 939 available");
            if (!bot.Quests.IsAvailable(939))
                {
                Console.WriteLine("Please Complete /battleunderc to run this bot.");
                StopBot("battleunderc");
                }
                    bot.Log($"[{DateTime.Now:HH:mm:ss}] is 2082 available");
                ...
                {
                    bot.Log($"[{DateTime.Now:HH:mm:ss}] Quest Complete P1");
                ...
                while (bot.Quests.CanComplete(Quest)) SafeQuestComplete(Quest);
                    ...
                    bot.Log($"[{DateTime.Now:HH:mm:ss}]while 2082 done complete");
                while (bot.Inventory.Contains("Undead Essence", 25)) bot.Quests.EnsureComplete(2082);
                    bot.Log($"[{DateTime.Now:HH:mm:ss}] bone terror");
                ItemFarm("itBone Terror Soulem", 1, false, true, 939, "Bone Terror", "battleundera");
                    bot.Log($"[{DateTime.Now:HH:mm:ss}] undead champ");
                ItemFarm("Undead Champion Soul", 1, false, true, 939, "Undead Champion", "battleunderb");
                    bot.Log($"[{DateTime.Now:HH:mm:ss}] crystalized jellyfish");
                ItemFarm("Jellyfish Soul", 1, false, true, 939, "Crystalized Jellyfish", "battleunderc");
                    bot.Log($"[{DateTime.Now:HH:mm:ss}] 939 complete");
                bot.Quests.EnsureComplete(939);
            }
