...
public class BluuPurpleTemplate
{
	...
	public void ScriptMain(ScriptInterface bot)
	{
		...
		while (!bot.ShouldExit())
		{
			while (!bot.Player.Loaded) { }
				 ...
				 if(bot.Inventory.Contains("Ceremonial Void Knight Sword", 1))		
				{	
					ItemFarm("Slugfit Horn", 5, true, true, 5662, "Slugfit", "mobius");
					ItemFarm("Imp Flame", 3, true, true, 5662, "Fire Imp", "relativity");
					ItemFarm("Aracara Silk", 1, true, true, 5662, "Aracara", "faerie");
					ItemFarm("Makai Fang", 5, true, true, 5662, "Dark Makai", "tercessuinotlim");
					ItemFarm("Aquamarine of Nulgath", 3, false, true, 5662, "Crystalized Jellyfish", "battleunderc");
					SafeQuestComplete(5662);							
				}				
				else
				{ 
				SafeMapJoin("Party");
				MessageBox.Show("Ceremonial Void Knight Sword RREQUIRED to run this bot");
				StopBot(); 
				}
		}
	...
	*/
	...
	public void StopBot(string Text = "Bot stopped successfully.", string MapName = "party", string CellName = "Enter", string PadName = "Spawn", string Caption = "Stopped", string MessageType = "event")
	{
		...
		bot.Options.AggroMonsters = false;
		bot.Log($"[{DateTime.Now:HH:mm:ss}] Taters Done ");
	...
	*/
	...
	/// </summary>
	public void ConfigureBotOptions(string PlayerName = "Bot By Tato", string GuildName = "https://auqw.tk/", bool LagKiller = true, bool SafeTimings = true, bool RestPackets = true, bool AutoRelogin = true, bool PrivateRooms = true, bool InfiniteRange = true, bool SkipCutscenes = true, bool ExitCombatBeforeQuest = true)
	{
		SendMSGPacket("Configuring bot.", "Tato", "Pobably not a monderator");
