...
public class BluuPurpleTemplate
{
	...
	public string SoloClass = "StoneCrusher"; //edit to your preference
	public string FarmClass = "Vampire Lord";	//edit to your preference
	public readonly int[] SkillOrder = { 2, 3, 1, 4 };
	//-----------EDIT ABOVE-------------//

	public int[] BPPQuests = {4703, 4704};
	public int[] DTQuests = {};
	...
	public string[] RequiredItems = { 
		...
		"Gem of Mastery",
		"Defeated Makai",
		"Void Highlord",//am lazy and test things that bank ac items so i added this
		"Vampire Lord"//am lazy and test things that bank ac items so i added this
	};
	...
	public void ScriptMain(ScriptInterface bot)
	{
		...
		while (!bot.ShouldExit())
		{
			...
			if(bot.Inventory.Contains("Legion Revenant")) StopBot("You already have the class, there's no more reason to run this script.");
			while(!bot.Quests.CanComplete(6900))
			{
				while(!bot.Inventory.Contains("Revenant's Spellscroll", 20))
				{
					bot.Quests.EnsureAccept(6897); //changed from 6987(mitsu bishi's gift)
					if(bot.Quests.CanComplete(6897)) SafeQuestComplete(6897);//changed from 6987(mitsu bishi's gift)
					bot.Sleep(200);
					
					SafeEquip(SoloClass);
					ItemFarm("Aeacus Empowered", 50, "judgement", "r10a", "Left");	
					SafeEquip(SoloClass);
					ItemFarm("Tethered Soul", 300, "revenant", "r2", "Left");

					SafeEquip(FarmClass);
					ItemFarm("Darkened Essence", 500, "shadowrealmpast");
					ItemFarm("Dracolich Contract", 1000, "necrodungeon", "r22", "Bottom");
				}
				bot.Log($"[{DateTime.Now:HH:mm:ss}] You've got 20x Revenant's Spellscroll.");
				...
				while(!bot.Inventory.Contains("Conquest Wreath", 6))
				{
					...
					SafeEquip(FarmClass);
					ItemFarm("Grim Cohort Conquered", 500, "doomvault", "r2", "Left");
					ItemFarm("Ancient Cohort Conquered", 500, "mummies");
					ItemFarm("Pirate Cohort Conquered", 500, "wrath", "r4", "Left");
					ItemFarm("Battleon Cohort Conquered", 500, "doomwar", "r8", "Down");
					ItemFarm("Mirror Cohort Conquered", 500, "overworld", "r2", "Down");
					ItemFarm("Darkblood Cohort Conquered", 500, "deathpits", "r2", "Left");
					ItemFarm("Vampire Cohort Conquered", 500, "maxius", "r2", "Left");
					ItemFarm("Spirit Cohort Conquered", 500, "cursedshore");
					ItemFarm("Dragon Cohort Conquered", 500, "dragonbone");
					ItemFarm("Doomwood Cohort Conquered", 500, "doomwood", "r6", "Right");
				}
				bot.Log($"[{DateTime.Now:HH:mm:ss}] You've got 6x Conquest Wreath.");
				...
				while(!bot.Inventory.Contains("Exalted Crown", 10))
				{
					...
					SafePurchase("Hooded Legion Cowl", 1, "underworld", 216);
					
					while(!bot.Inventory.Contains("Legion Token", 4000))
					{
						if(bot.Inventory.Contains("Bright Paragon Pet"))
						{
							MultiQuestFarm("brightfortress", "Fortress3", "Left", BPPQuests);
						}
						...
						else
						{
							...
							if(bot.Quests.CanComplete(4849)) SafeQuestComplete(4849);
							bot.Sleep(200);
							
							SafePurchase("Undead Champion", 1, "underworld", 216);
							
							ItemFarm("Dreadrock Enemy Recruited", 6, "dreadrock", "r3", "Bottom", false);
						}
					}
					
					SafeEquip(FarmClass);
					ItemFarm("Dage's Favor", 300, "evilwardage", "r9", "Left");
					
					while(!bot.Inventory.Contains("Emblem of Dage"))
					{
						bot.Quests.EnsureAccept(4742);
						if(bot.Quests.CanComplete(4742)) SafeQuestComplete(4742);
						bot.Sleep(200);
						
						SafeEquip(FarmClass);
						ItemFarm("Fiend Seal", 25, "shadowblast", "r13", "Left");
						ItemFarm("Gem of Mastery", 1, "shadowblast", "r10", "Left");
					}
					
					while(!bot.Inventory.Contains("Diamond Token of Dage", 30))
					{
						bot.Quests.EnsureAccept(4743);
						if(bot.Quests.CanComplete(4743)) SafeQuestComplete(4743);
						bot.Sleep(200);
						
						SafeEquip(FarmClass);
						ItemFarm("Defeated Makai", 25, "tercessuinotlim", "m2", "Bottom");
						
						SafeEquip(SoloClass);
						ItemFarm("Carnax Eye", 1, "aqlesson", "Frame9", "Right", true);
						ItemFarm("Kathool Tentacle", 1, "deepchaos", "Frame4", "Left", true);
						ItemFarm("Red Dragon's Fang", 1, "lair", "End", "Right", true);
						ItemFarm("Fluffy's Bones", 1, "dflesson", "r13", "Right", true);
						ItemFarm("Blood Titan's Blade", 1, "bloodtitan", "Enter", "Spawn", true);
					}
					
					while(!bot.Inventory.Contains("Dark Token", 100))
					{
						MultiQuestFarm("seraphicwardage", "r3", "Right", DTQuests);
					}
				}
			}
		}
		bot.Log($"[{DateTime.Now:HH:mm:ss}] Script stopped successfully.");
		StopBot("Congratulations! \nYou got: \n20x Revenant's Spellscroll \n6x Conquest Wreath \n10x Exalted Crown \nNow you just need to turn in the Legion Fealty 4 Quest.");
	}
	...
	public void SpringCleaning(int QuestID)
	{
		...
		if(bot.Quests.CanComplete(QuestID)) SafeQuestComplete(QuestID);
		bot.Sleep(200);
		
		SafeEquip(FarmClass);
		ItemFarm("Nothing Heard", 40, "fotia", "Enter", "Spawn", false);
		ItemFarm("Nothing To See", 40, "fotia", "Enter", "Spawn", false);
		ItemFarm("Area Secured and Quiet", 40, "fotia", "Enter", "Spawn", false);
	}
	...
	*/
	...
	{
		SendMSGPacket("Configuring bot.", "Tato", "moderator");
