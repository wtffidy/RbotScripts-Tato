...
public class BluuTemplate
{
	...
	public void ScriptMain(ScriptInterface bot)
	{
		...
		while (!bot.ShouldExit())
		{
			...
			while (bot.Player.GetFactionRank("Brightoak") < 10)
            {
				SafeMapJoin("elfhame", "Cut1", "Left");
				bot.SendPacket("%xt%zm%getMapItem%95442%3984%");
				if (bot.Quests.CanComplete(4667)) SafeQuestComplete(4667);
            }
		}
