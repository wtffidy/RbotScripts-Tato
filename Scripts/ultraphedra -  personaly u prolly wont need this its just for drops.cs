...
public class BluuPurpleTemplate
{
	...
	public int[] QuestList = { 4234, 4235};
	public string[] RequiredItems = { 
        "Flame Titan Gryphon",
        "Ultra Flame Titan Spirit",
        "Ultra Flame Titan Spirit Pet (Free Player)",
        "Phedra Feather"
    };
	...
	public void ScriptMain(ScriptInterface bot)
	{
		...
		while (!bot.ShouldExit())
		{
           ...
           // MultiQuestFarm(string MapName, string CellName, string PadName, int[] QuestList, string MonsterName = "*")
            MultiQuestFarm("lavarun", "r7", "Left", QuestList, "*");
           	//SafePurchase("ItemName", ItemQuantityNeeded, "MapName", "MapNumber", ShopID)
           	SafePurchase("Ultra Flame Titan Spirit Pet", 1, "lavarun", 1143);
		}
		bot.Log($"[{DateTime.Now:HH:mm:ss}] gotcho shiz");
		StopBot();
	}
