...
public class ShadowFortressQuestlineForFarming
{
	//-----------EDIT BELOW-------------//
	...
	public string[] RequiredItems = { "Perfect Orochi Scales", "Dragon Shinobi Token" };
	...
	public void ScriptMain(ScriptInterface bot)
	{
		...
		while (!bot.ShouldExit())
		{
            ...
            while(!bot.Inventory.Contains("Dragon Shinobi Token", 300))
            {
    ...
    public void QuestLine()
		{ 					
		bot.Log($"[{DateTime.Now:HH:mm:ss}] EightHeadsOfOrochiAIOstart");
		EightHeadsOfOrochiAIO:  //EightHeadsOfOrochi
            if (bot.Quests.IsAvailable(6494))
                {	goto DefeatJaakuu;
                    if (bot.Quests.IsAvailable(6493))
                        {	goto FindJaakuu;
                        if (bot.Quests.IsAvailable(6492))
                                {	goto GetToTheRoof;	//FindJaakuu								
                                if (bot.Quests.IsAvailable(6487))
                                        {	goto FindTheThroneRoom;											
                                        if (bot.Quests.IsAvailable(6486))
                                                {	goto DisarmTheSamurai;
                                                if (bot.Quests.IsAvailable(6485))
                                                        {	goto CalmTheSpirits;
                                                }}}}}}
			CalmTheSpirits:
            	bot.Log($"[{DateTime.Now:HH:mm:ss}] Ghosts");
				ItemFarm("Restless Spirit Calmed", 8, true, true, 6485, "Restless Spirit", "shadowfortress");
				ExitCombat();
				bot.Quests.EnsureComplete(6485);
                
			    goto DisarmTheSamurai;

            DisarmTheSamurai:
            	bot.Log($"[{DateTime.Now:HH:mm:ss}] Disarmed");
				ItemFarm("Samurai Disarmed", 8, true, true, 6486, "Shadow Samurai", "shadowfortress");
				ExitCombat();
				bot.Quests.EnsureComplete(6486);
				goto FindTheThroneRoom;

            FindTheThroneRoom:
                bot.Quests.EnsureAccept(6487);
                bot.Sleep(2500);
                bot.Log($"[{DateTime.Now:HH:mm:ss}] mapmove");
				SafeMapJoin("shadowfortress");
                bot.Sleep(500);
                bot.Player.Jump("r3", "Left");
                bot.Sleep(500);
                bot.Player.Jump("r4", "Left");
                bot.Sleep(500);
                if (bot.Quests.CanComplete(6487)) SafeQuestComplete(6487);
			    goto GetToTheRoof;
                

			GetToTheRoof:
                bot.Quests.EnsureAccept(6492);
                bot.Sleep(2500);
                bot.Log($"[{DateTime.Now:HH:mm:ss}] mapmove2");
				SafeMapJoin("shadowfortress");
                bot.Player.Jump("r8c", "Left");
                bot.Sleep(500);
                bot.SendPacket("%xt%zm%getMapItem%7622%5979%"); 
                bot.Sleep(500);
                bot.Player.Jump("r9", "Left");
                bot.Sleep(500);
                if (bot.Quests.CanComplete(6492)) SafeQuestComplete(6492);
				goto FindJaakuu;
                

			FindJaakuu:
            	bot.Log($"[{DateTime.Now:HH:mm:ss}] Give good head");
				ItemFarm("6th Head Defeated", 1, true, true, 6493, "6th Head of Orochi", "shadowfortress");
				ItemFarm("5th Head Defeated", 1, true, true, 6493, "5th Head of Orochi", "shadowfortress");
				ItemFarm("4th Head Defeated", 1, true, true, 6493, "4th Head of Orochi", "shadowfortress");
				ItemFarm("3rd Head Defeated", 1, true, true, 6493, "3rd Head of Orochi", "shadowfortress");
				ItemFarm("2nd Head Defeated", 1, true, true, 6493, "2nd Head of Orochi", "shadowfortress");
				ItemFarm("1st Head Defeated", 1, true, true, 6493, "1st Head of Orochi", "shadowfortress");
				ExitCombat();
				bot.Quests.EnsureComplete(6493);
                goto DefeatJaakuu;	

			DefeatJaakuu:
                bot.Log($"[{DateTime.Now:HH:mm:ss}] killed the samurai");
				ItemFarm("Jaaku Defeated", 1, true, true, 6494, "Jaaku", "shadowfortress");
				ExitCombat();
				bot.Quests.EnsureComplete(6494);
			    goto EightHeadsOfOrochi;

			EightHeadsOfOrochi:
                bot.Log($"[{DateTime.Now:HH:mm:ss}] InsertTexthere");
				ItemFarm("Perfect Orochi Scales", 10, false, true, 7924, "1st Head Of Orochi|2nd Head Of Orochi|3rd Head Of Orochi|4th Head Of Orochi|5th Head Of Orochi|6th Head Of Orochi|7th Head Of Orochi", "shadowfortress");
				ExitCombat();
				bot.Quests.EnsureComplete(7924);		
		}
